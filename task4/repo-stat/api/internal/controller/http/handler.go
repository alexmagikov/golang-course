...
func NewHandler(ctx context.Context, log *slog.Logger, cfg config.Config) (http.Handler, error) {
	...
	subscriptionsProviderUsecase := usecase.NewSubscriptionProvider(subscriberClient, processorClient)

	mux := http.NewServeMux()
	AddRoutes(mux, log, pingUseCase, repoUseCase, subscriptionsProviderUsecase)

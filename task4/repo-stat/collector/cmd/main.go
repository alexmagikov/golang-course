...
func run(ctx context.Context) error {
	...
	if err != nil {
		log.Error("error creating subscriber client", "error", err)
		return err
	}
	defer func() {
	...
	collectorUseCase := usecase.NewCollectorProvider(log, githubClient, subscriberClient)

	collectorServer := grpcController.NewCollectorServer(collectorUseCase)
	...
	collector.RegisterCollectorServiceServer(grpcServer, collectorServer)
